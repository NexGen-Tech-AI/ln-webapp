...
export function SocialLoginButtons({ mode = 'login', className = '' }: SocialLoginButtonsProps) {
  ...
  const providers = [
    ...
    },
    {
      ...
      bgColor: 'bg-[#2F2F2F] hover:bg-[#262626] text-white'
    },
    {
      name: 'LinkedIn',
      ...
      bgColor: 'bg-[#0A66C2] hover:bg-[#004182] text-white'
    },
    {
      name: 'Twitter',
  ...
  const handleSocialLogin = async (provider: typeof providers[0]['provider']) => {
    setLoadingProvider(provider)
    
    try {
      const result = await apiService.signInWithOAuth(provider)
      ...
      <div className="grid gap-2">
        {providers.map((provider) => (
          <Button
            ...
            className={`relative ${provider.bgColor} ${
              loadingProvider === provider.provider ? 'opacity-80' : ''
            }`}
          >
            ...
            {mode === 'signup' ? `Sign up with ${provider.name}` : `Sign in with ${provider.name}`}
          </Button>
        ))}
      </div>
